...
def generate_hashes():
    ...
    scrypt_hash = hashlib.scrypt(
        password.encode("utf-8"), salt=scrypt_salt, n=16384, r=8, p=1
    ).hex()
    ...
    if algorithm == "auto":
        ...
        if not candidates:
            return (
                jsonify(
                    {
                        "error": "Could not identify hash format. Please specify algorithm manually."
                    }
                ),
                400,
            )
        ...
        for cand in candidates:
            total_log.append(
                {
                    "word": f"--- Trying detected algorithm: {cand} ---",
                    "hash": "",
                    "status": "INFO",
                    "found": False,
                }
...
def _run_crack_logic(target_hash, algorithm, words):
    ...
    for word in words:
        ...
        if algorithm == "sha256":
            ...
            word_hash = hashlib.scrypt(
                word.encode("utf-8"), salt=b"hashlab_salt", n=16384, r=8, p=1
            ).hex()
            match = word_hash == target_hash
        elif algorithm == "bcrypt":
            try:
                match = bcrypt.checkpw(
                    word.encode("utf-8"), target_hash.encode("utf-8")
                )
            ...
            log.append(
                {
                    "word": word,
                    "hash": (
                        word_hash[:20] + "..." if len(word_hash) > 20 else word_hash
                    ),
                    "status": "CRACKED",
                    "found": True,
                }
